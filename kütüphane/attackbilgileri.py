...
def start(target, port, authmethod, foundRoutes=[]):
    ...
    if len(foundRoutes) > 0: # Rota öncelikli cihazlardan akışlar bulundu
        finalRet = [] # Akış URL'leri ile nihai dönüş
        authDetected = dealer.decide(target, port, foundRoutes[0]) # Yetkilendirme yöntemini algıla, çünkü burada yalnızca "Yok" yetkimiz var
        foundUser = ""
        foundPassword = ""
        ...
        try:
            ...
            sock.settimeout(20) # 3 saniyelik zaman aşımına izin veriyoruz
            sock.connect((target, port)) # Çift parantez gerekli
        ...
        finalRet = dict() # Akış URL'leri ile nihai dönüş
        try:
            # Hedefe bir döngü içinde bağlanırız (bazı DIGEST AUTH cihazları bir rota bulunursa bağlantıyı keser)
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(20) #3 saniyelik zaman aşımına izin veriyoruz
            ...
            for pwd in userpasslist['passwords']:
                try:
                    ...
                    if authmethod == "Digest":
                        ...
                        # Özet yanıtı alın (nonce, realm vb.)
                        sock.send(describe(descURL, sequence))
                        digestBuffer = sock.recv(1024).decode()
                        sequence += 1
